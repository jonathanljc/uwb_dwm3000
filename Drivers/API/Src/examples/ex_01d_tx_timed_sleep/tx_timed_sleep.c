...
volatile int sleeping = 0; /* this will be set to 1 in the main as device is put to sleep, and it will be cleared in the callback once device wakes up */
...
int tx_timed_sleep(void)
{
    ...
    while (1)
    {
        ...
        /* Because we are using TX only, we only restore the configurations related to TX. */
        if(dwt_restore_txrx(DWT_RESTORE_TX_ONLY_MODE) != (int32_t)DWT_SUCCESS)
        {
            test_run_info((unsigned char *)"RESTORE TXRX FAILED     ");
            while (1) { };
        }
...
static void spi_ready_cb(const dwt_cb_data_t *cb_data)
{
    ...
    if(!(DWT_INT_RCINIT_BIT_MASK & cb_data->status))
    {
